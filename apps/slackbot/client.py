...
@app.command("/philfinger")
def finger(ack, say, command):
    ...
    slack_user_info = webclient.users_info(user=command["user_id"])
...
```{finger_output}```""",
        ...
        say(f"{slack_user_name} tried to finger {csua_user} but failed!")


@app.command("/man")
def man(ack, say, command):
