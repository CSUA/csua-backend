...
def get_events_in_time_range(range):
    ...
    """
    events = Event.objects.filter(date_time__gte=range[0]) \
                          .filter(date_time__lte=range[1])
    return events
...
def timeify(requested_tdelta):
    ...
    end_times = {
        ...
        "tomorrow": (now + datetime.timedelta(days=1)).replace(hour=23, minute=59, second=59),
